...
users run**. It ships no source code: it is a Home Assistant add-on manifest
...
**If you are looking for GLP, you want the stable add-on instead:**
...
database, and carries no upgrade path — the stable add-on will not import
anything from it. There are no release notes and no support for this channel.

## How it coexists with the stable add-on

The dev add-on uses a different slug (`gaggiuino_local_profiler_dev`), so Home
Assistant treats it as a completely separate add-on:
...
Separate slugs mean separate persistent storage, so the dev add-on can never
write to the stable add-on's shot archive or coffee library. Both can run at
the same time. Point them at the same machine only if you accept that both
will sync shots from it.

## Maintainer setup

1. Home Assistant → Settings → Add-ons → Add-on Store → ⋮ → Repositories
2. Add `https://github.com/mxkissnr/glp-dev-addon`
3. Install **GLP DEV**
